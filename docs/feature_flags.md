...
**注意**: このプロジェクトは学習用途のみです。商用利用は禁止されています。

---

## 🚩 利用可能なフィーチャーフラグ

### ENABLE_BILLING（学習用決済機能）

**用途**: 学習用決済機能の有効/無効を制御（学習用途のみ）

**設定値**:
- `True` - 学習用決済機能を有効化（開発環境推奨）
- `False` - 学習用決済機能を無効化（デフォルト推奨）

**影響範囲**:
- バックエンド: 学習用決済関連エンドポイントへのアクセス
- フロントエンド: 学習用決済関連UIの表示/非表示

**重要**: この機能は学習目的のみであり、実際の決済処理は行われません
